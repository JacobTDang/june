...
export async function addByLink(roomId: string, url: string): Promise<YouTubeResult<void>> {
...
export async function getPlaylistTracks(playlistId: string): Promise<YouTubeResult<VideoMeta[]>> {
  return runYouTube(async () => {
...
export async function addVideoById(roomId: string, videoId: string): Promise<YouTubeResult<void>> {
  return runYouTube(async () => {
    const youtube = await youtubeClient();
    const [meta] = await getVideoMetas([videoId], supabaseVideoCache(youtube));
    if (!meta) throw new Error("Couldn't find that video.");
...
export async function listMyPlaylists(): Promise<
  YouTubeResult<{ id: string; title: string; itemCount: number; thumbnailUrl: string | null }[]>
> {
  return runYouTube(async () => {
    const youtube = await youtubeClient(true);
