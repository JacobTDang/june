...
// The iframe_api bootstrap injects this widget script; route it through us too,
// in case the blocker also filters /s/player/ paths.
const WIDGET_URL_RE =
  /https:\/\/www\.youtube\.com(\/s\/player\/[A-Za-z0-9_/-]+\/www-widgetapi\.js)/g;

/** Rewrite the widget-script URL inside iframe_api to point at our proxy. */
export function rewriteWidgetUrls(js: string): string {
  return js.replace(WIDGET_URL_RE, (_m, path: string) => `/api/yt-widget?path=${encodeURIComponent(path)}`);
}

/** SSRF guard: only ever proxy a genuine YouTube widget-script path. */
export function isValidWidgetPath(path: string): boolean {
  return /^\/s\/player\/[A-Za-z0-9_/-]+\/www-widgetapi\.js$/.test(path);
}
